...
export default function ChatInterface({ messages, onMessagesUpdate, onCanGenerate, onMarketplaceSearch, showMarketplace = false }: ChatInterfaceProps) {
  ...
  const renderSourceFiles = (message: ChatMessage) => {
    ...
    if (message.sourcesUsed && message.sourcesUsed.length > 0) {
      ...
      return (
        <div className="mt-2 pt-2 border-t border-slate-600">
          <div className="text-xs text-slate-300 mb-2 font-medium">📚 Content Sources Used:</div>
    ...
    if (message.sourceFiles && message.sourceFiles.length > 0) {
      return (
        <div className="mt-2 pt-2 border-t border-slate-600">
          <div className="text-xs text-slate-300 mb-1">Referenced files:</div>
          <div className="flex flex-wrap gap-1">
            {message.sourceFiles.map((file, idx) => (
              <span key={idx} className="inline-flex items-center gap-1 bg-slate-600 px-2 py-1 text-xs border border-slate-500 text-white rounded">
          ...
          >
            <div className={`flex gap-3 max-w-[80%] ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-8 h-8 rounded-full border-2 border-slate-600 flex items-center justify-center ${
                message.role === 'user' ? 'bg-blue-600' : 'neopop-gradient-primary'
              }`}>
                {message.role === 'user' ? <FaUser className="text-sm text-white" /> : <FaRobot className="text-sm text-white" />}
              </div>
              
              <div className={`border-2 border-slate-600 p-3 rounded ${
                message.role === 'user' 
                  ? 'bg-blue-900 brutal-shadow-right text-white' 
                  : 'bg-slate-700 brutal-shadow-left text-white'
              }`}>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="flex gap-3">
              <div className="w-8 h-8 rounded-full border-2 border-slate-600 neopop-gradient-primary flex items-center justify-center">
                <FaSpinner className="text-sm animate-spin text-white" />
              </div>
              <div className="border-2 border-slate-600 bg-slate-700 p-3 brutal-shadow-left rounded">
                <div className="font-freeman text-sm text-slate-300">
                  AI is thinking...
                </div>
              </div>
            </div>
      ...
      <div className="p-4 bg-slate-700 border-t border-slate-600">
        ...
        {!showMarketplace && (
          <div className="mb-3 flex justify-center">
            <button
              ...
              className="neopop-gradient-secondary border-2 border-slate-600 brutal-shadow-center hover:translate-y-1 transition-all px-4 py-2 font-freeman font-bold text-white rounded hover:neopop-glow"
            >
        ...
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 border-2 border-slate-600 p-3 font-freeman resize-none bg-slate-600 text-white placeholder-slate-400 rounded"
            rows={2}
            disabled={isLoading}
          />
          <button
            ...
            className="neopop-gradient-primary border-2 border-slate-600 brutal-shadow-center hover:translate-y-1 transition-all p-3 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded hover:neopop-glow"
          >
            <FaPaperPlane />
          </button>
        </div>
        
        <div className="mt-2 text-xs text-slate-400 font-freeman">
